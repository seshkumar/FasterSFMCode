...
#include "cyclicBCDTruncTV6N3D.h"
//#include "mex.h"

CyclicBCDTruncTV6N3D::CyclicBCDTruncTV6N3D(Graph6N3D &g, double epsilon)
{
    initialize(g.y, g.W1, g.W2, g.W3, g.u, g.dims, epsilon);
}

CyclicBCDTruncTV6N3D::CyclicBCDTruncTV6N3D(double ***y, double **W1, double **W2, double **W3, double ***u, const size_t *dims, double epsilon)
{
    initialize(y, W1, W2, W3, u, dims, epsilon);
}

void CyclicBCDTruncTV6N3D::initialize(double ***y, double **W1, double **W2, double **W3, double ***u, const size_t *dims, double epsilon)
{
    ...
    projL1 = new TruncDualLineProjector(d*h, n                );
    projL2 = new TruncDualLineProjector(w*d, n + (d*h)        );
    projL3 = new TruncDualLineProjector(w*h, n + (d*h) + (w*d));
...
CyclicBCDTruncTV6N3D::~CyclicBCDTruncTV6N3D()
{
    delete[] n;
    delete projL1;
    delete projL2;
...
void CyclicBCDTruncTV6N3D::parallelAccumulate(double ** tb, double ** ta, size_t dir)
{
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
...
void CyclicBCDTruncTV6N3D::computeGapsUpdateSolution(const size_t iter, std::vector<OptInfo> &stats)
{
    double **x1 = tb;
    double **x2 = x1+(d*h);
    double **x3 = x2+(w*d);
        ...
        for(int i=0; i < w; ++i)
            for(int j=0; j < h; ++j)
                for(int k=0; k < d; ++k)
                {
                    ...
                    if(fabs(sijk) <= epsilon)
                        dValue -= (sijk*sijk)/(2*epsilon);
                    else
                        dValue -= (fabs(sijk) - 0.5*epsilon);

                    pValue += (yijk*yijk*epsilon/2 - yijk*u[i][j][k]);
        ...
        for(int i=0; i < w; ++i)
            for(int j=0; j < h; ++j)
                for(int k=0; k < d; ++k)
                {
                    ...
                    if(i < w-1)
                        pValue += fabs(y[i][j][k] - y[i+1][j][k])*W1[k*h + j][i];

                    if(j < h-1)
                        pValue += fabs(y[i][j][k] - y[i][j+1][k])*W2[k*w + i][j];
                        
                    if(j < d-1)
                        pValue += fabs(y[i][j][k] - y[i][j][k+1])*W3[j*w + i][k];
                }
    }
...
double CyclicBCDTruncTV6N3D::greedyTV(void)
{
    const double **W1 = W;
    const double **W2 = W1+(d*h);
    const double **W3 = W2+(w*d);
...
void CyclicBCDTruncTV6N3D::optimize(const size_t maxIterations, std::vector<OptInfo> & stats, char * fname)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        if(fabs(primal_sfm - dual_sfm) < 1e-3)
            break;
    }
}

void CyclicBCDTruncTV6N3D::optimize_var(const size_t maxIterations, std::vector<OptInfo> & stats, char * fname)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        parallelAccumulate(ta, tb, 1);
        projL1->parallelProject(z1, tb1, ta1, W1, u2D1, epsilon/(iter+1));

        for(size_t i=0; i < d*h; ++i)
            nIter1 = (nIter1 > projL1->iter[i])? nIter1: projL1->iter[i];
                                                      
        parallelAccumulate(ta, tb, 2);                
        projL2->parallelProject(z2, tb2, ta2, W2, u2D2, epsilon/(iter+1));
                                                      
        for(size_t i=0; i < w*d; ++i)
            nIter2 = (nIter2 > projL2->iter[i])? nIter2: projL2->iter[i];

        parallelAccumulate(ta, tb, 3);                
        projL3->parallelProject(z3, tb3, ta3, W3, u2D3, epsilon/(iter+1));
...
void CyclicBCDTruncTV6N3D::optimize_var_sqrt(const size_t maxIterations, std::vector<OptInfo> & stats, char * fname)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        parallelAccumulate(ta, tb, 1);
        projL1->parallelProject(z1, tb1, ta1, W1, u2D1, epsilon/sqrt(iter+1));

        for(size_t i=0; i < d*h; ++i)
            nIter1 = (nIter1 > projL1->iter[i])? nIter1: projL1->iter[i];
                                                      
        parallelAccumulate(ta, tb, 2);                
        projL2->parallelProject(z2, tb2, ta2, W2, u2D2, epsilon/sqrt(iter+1));
                                                      
        for(size_t i=0; i < w*d; ++i)
            nIter2 = (nIter2 > projL2->iter[i])? nIter2: projL2->iter[i];

        parallelAccumulate(ta, tb, 3);                
        projL3->parallelProject(z3, tb3, ta3, W3, u2D3, epsilon/sqrt(iter+1));
...
//void CyclicBCDTruncTV6N3D::optimize(double ** start, const size_t maxIterations, std::vector<OptInfo> & stats)
//{
...
double CyclicBCDTruncTV6N3D::duplicate(double **x, double *** u)
{
    double **x1 = x;
    double **x2 = x1+(d*h);
    double **x3 = x2+(w*d);
