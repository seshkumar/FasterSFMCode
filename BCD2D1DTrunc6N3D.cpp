...
BCD2D1DTrunc6N3D::BCD2D1DTrunc6N3D(Graph6N3D &g, double epsilon)
{
    initialize(g.y, g.W1, g.W2, g.W3, g.u, g.dims, epsilon);
}

BCD2D1DTrunc6N3D::BCD2D1DTrunc6N3D(double ***y, double **W1, double **W2, double **W3, double ***u, const size_t *dims, double epsilon)
{
    initialize(y, W1, W2, W3, u, dims, epsilon);
}

void BCD2D1DTrunc6N3D::initialize(double ***y, double **W1, double **W2, double **W3, double ***u, const size_t *dims, double epsilon)
{
    ...
    projF = new TruncDualFrameProjector(nThreadsF, w, h);
    projL = new TruncDualLineProjector (nThreadsL, n);
...
BCD2D1DTrunc6N3D::~BCD2D1DTrunc6N3D()
{
    delete[] n;
    delete[] m;
...
void BCD2D1DTrunc6N3D::parallelAccumulate(size_t dir, double beta)
{
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
...
void BCD2D1DTrunc6N3D::computeGapsUpdateSolution(const size_t iter, std::vector<OptInfo> &stats)
{
    double **x1 = this->ta;
    double **x2 = x1+(w*d);
...
double BCD2D1DTrunc6N3D::greedyTV(void)
{
    const double **W1 = W;
    const double **W2 = W1+(d*h);
    const double **W3 = W2+(w*d);
...
void BCD2D1DTrunc6N3D::optimize(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        projF->parallelProject(z1, ta1, tb1, W1, W2, u2D1, epsilon);
        parallelAccumulate(2);

        for(size_t i=0; i < nThreadsF; ++i)
            nIterF = (nIterF > projF->iter[i])? nIterF: projF->iter[i];
                                                      
        projL->parallelProject(z2, ta2, tb2, W3, u2D2, epsilon);
...
void BCD2D1DTrunc6N3D::optimize_acc(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
...
void BCD2D1DTrunc6N3D::optimize_var(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        projF->parallelProject(z1, ta1, tb1, W1, W2, u2D1, epsilon/(iter+1));
        parallelAccumulate(2);

        for(size_t i=0; i < nThreadsF; ++i)
            nIterF = (nIterF > projF->iter[i])? nIterF: projF->iter[i];
                                                      
        projL->parallelProject(z2, ta2, tb2, W3, u2D2, epsilon/(iter+1));
        parallelAccumulate(1);
...
void BCD2D1DTrunc6N3D::optimize_var_acc(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
...
void BCD2D1DTrunc6N3D::optimize_var_sqrt(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
    ...
    for(int iter =0; iter < maxIter; ++iter)
    {
        ...
        projF->parallelProject(z1, ta1, tb1, W1, W2, u2D1, epsilon/sqrt(iter+1));
        parallelAccumulate(2);

        for(size_t i=0; i < nThreadsF; ++i)
            nIterF = (nIterF > projF->iter[i])? nIterF: projF->iter[i];
                                                      
        projL->parallelProject(z2, ta2, tb2, W3, u2D2, epsilon/sqrt(iter+1));
        parallelAccumulate(1);
...
void BCD2D1DTrunc6N3D::optimize_var_sqrt_acc(const size_t maxIterations, std::vector<OptInfo> & stats)
{   
    size_t w = this->w;
    size_t h = this->h;
    size_t d = this->d;
...
//void BCD2D1DTrunc6N3D::optimize(double ** start, const size_t maxIterations, std::vector<OptInfo> & stats)
//{
...
double BCD2D1DTrunc6N3D::duplicate(double **x, double *** u)
{
    double **x1 = x;
    double **x2 = x1+(w*d);
